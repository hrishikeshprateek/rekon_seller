...
class DeliveryFilterPage extends StatefulWidget {
  final List<Map<String, dynamic>>? initialSelectedFilters;

  const DeliveryFilterPage({Key? key, this.initialSelectedFilters}) : super(key: key);

  @override
  State<DeliveryFilterPage> createState() => _DeliveryFilterPageState();
}

class _DeliveryFilterPageState extends State<DeliveryFilterPage> {
  ...
  Future<void> _loadFilters() async {
    try {
      ...
      debugPrint('[DeliveryFilter] Loading filters with payload: $payload');
      ...
      if (parsed['success'] == true && parsed['data'] != null) {
        ...
        // Filter out Station (id 1), keep only Area (id 2) and Route (id 3)
        _categories = cats
            .map((c) => FilterCategory.fromJson(c as Map<String, dynamic>))
            .where((cat) => cat.id != 1) // Remove Station
            .toList();
        ...
        // If we have initial selected filters passed from caller, apply them
        ...
        debugPrint('[DeliveryFilter] Loaded ${_categories.length} categories');
      } else {
        _error = parsed['message']?.toString() ?? 'Failed to load filters';
      }
    } catch (e) {
      debugPrint('[DeliveryFilter] Error loading filters: $e');
      _error = e.toString();
    } finally {
